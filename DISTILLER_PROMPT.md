...
Every atom must contain a verbatim transcript quote on a `**Evidence:**` line. Never paraphrase or invent evidence. Quotes must come from USER or ASSISTANT dialogue — SESSION SUMMARY blocks are context only, never evidence (they are model-generated recollection, not primary record; a quote from one will fail verification). Feedback atoms must also include `**Why:**` and `**How to apply:**`.
